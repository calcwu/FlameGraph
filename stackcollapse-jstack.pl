...
#		goto clear if $state ne "BLOCKED";
		# goto clear if $state ne "TIMED_WAITING";
#		goto clear if $state eq "RUNNABLE";
#		goto clear if $state eq "BLOCKED";
#		goto clear if $state eq "TIMED_WAITING";
#		goto clear if $state eq "WAITING";

		# save stack
		if (defined $tname) { unshift @stack, $tname; }
        my $statesuffix = ($state eq "RUNNABLE") ? "_${state}_[i]" : "_${state}_[j]";
		remember_stack(join(";", @stack).$statesuffix, 1) if @stack;
